...
class Trader:
    """
    v44 — ASH directional mode (passive-only variant).
    When |position| > DIRECTIONAL_THRESHOLD (40):
      - Hard-stop PASSIVE bids/asks on the adding side (penny-jump quotes only)
      - Keep _take_mispriced on both sides always (cheap asks below fair are our alpha)
      - Tighten inner unwind quote by 1 tick for faster inventory mean-reversion
    v43 lesson: suppressing _take_mispriced buys at 40 blocked our alpha (buying cheap below fair).
    This version only suppresses the passive quoting side while keeping mispricing capture intact.
    """
    ...
    def _trade_osmium(self, state: TradingState, fair_value, current_mid, last_mid) -> List[Order]:
        ...
        # _take_mispriced: always take both sides — cheap asks below fair are our edge
