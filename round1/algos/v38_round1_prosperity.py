...
class Trader:
    """
    Round 2 Strategy — v38
    ============================================
    Assets: ASH_COATED_OSMIUM (Pegged/Stationary), INTARIAN_PEPPER_ROOT (Trend Carry)

    Change from v35:
      - EMERGENCY_THRESHOLD: 60 -> 50, EMERGENCY_TARGET: 30 -> 25
      - Rationale: v36 log shows ASH position spent 26.8% of time at extremes (|pos|>=30)
        and only 11.3% near zero. At extremes the quote skew reduces fill rate. Unwinding
        sooner (at 50 instead of 60) keeps position closer to neutral more often, improving
        spread capture rate. Target also reduced proportionally.
    """
    ...
    EMERGENCY_THRESHOLD = {"ASH_COATED_OSMIUM": 50}
    EMERGENCY_TARGET = {"ASH_COATED_OSMIUM": 25}
