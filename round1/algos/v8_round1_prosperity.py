...
class Trader:
    ...
    IMB_ALPHA = {"ASH_COATED_OSMIUM": 0.25, "INTARIAN_PEPPER_ROOT": 0.20}
    IMB_MAX_TICKS = 2
    IMB_THRESHOLD_SHIFT = 1.0
    ...
    def _trade_pepper_root(self, state: TradingState, adjusted_history: list, base_estimate) -> List[Order]:
        ...
        imb_shift = self.IMB_THRESHOLD_SHIFT * abs(imb)
        buy_threshold = max(0.5, self.PEPPER_SIGNAL_THRESHOLD - inventory_shift - imb_shift)
        sell_threshold = max(0.5, self.PEPPER_SIGNAL_THRESHOLD + inventory_shift - imb_shift)
        ...
        if signal <= -buy_threshold:
            ...
            bid_price = self._passive_bid_price(
                best_bid,
                best_ask,
                fair_center - bid_distance + skew,
                passive_bid_cap,
            )
            ask_price = self._passive_ask_price(
                best_bid,
                best_ask,
                fair_center + ask_distance + skew,
                passive_ask_floor,
            )
