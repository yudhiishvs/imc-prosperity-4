...
class Trader:
    ...
    IMB_ALPHA = {"ASH_COATED_OSMIUM": 0.10, "INTARIAN_PEPPER_ROOT": 0.05}
    IMB_MAX_TICKS = 2
    ...
    def _penny_jump_quotes(
        ...
        penny_bid = min(penny_bid + skew, fair - 1)
        penny_ask = max(penny_ask + skew, fair + 1)
    ...
    def _trade_pepper_root(self, state: TradingState, adjusted_history: list, base_estimate) -> List[Order]:
        ...
        if signal <= -buy_threshold:
            ...
            bid_price = self._inside_bid(bb, ba, bid_inside, fair_center - bid_distance + skew)
            ask_price = self._inside_ask(bb, ba, ask_inside, fair_center + ask_distance + skew)
