...
class Trader:
    ...
    Round 2 Strategy
    ============================================
    Assets: ASH_COATED_OSMIUM (Pegged/Stationary), INTARIAN_PEPPER_ROOT (Fast Mean-Reversion)
    """
    ...
    # ── PEPPER_ROOT Configuration (Placeholders) ────────────────────────
    PEPPER_EMA_ALPHA = 0.5
    PEPPER_PRICE_HISTORY_LENGTH = 13
    PEPPER_SIGNAL_THRESHOLD = 2.0
    PEPPER_INVENTORY_SKEW = 0.08
    PEPPER_EMERGENCY_THRESHOLD = 65
    PEPPER_EMERGENCY_TARGET = 45

    PEPPER_WARMUP_TICKS = 10
    PEPPER_WARMUP_DISTANCE = 5
    PEPPER_WARMUP_SIZE = 4
    PEPPER_PASSIVE_DISTANCE = 3
    PEPPER_PASSIVE_SIZE = 6

    # ── Required by Prosperity ─────────────────────────────────────────
    def bid(self) -> int:
        return 15
    ...
    def _load_data(self, raw: str) -> dict:
        if not raw:
            return {"pepper_prices": [], "pepper_ema": None}
        try:
        ...
        cleaned_prices = cleaned_prices[-self.PEPPER_PRICE_HISTORY_LENGTH:]

        pepper_ema = data.get("pepper_ema")
        if not isinstance(pepper_ema, (int, float)):
            pepper_ema = None
        else:
            pepper_ema = float(pepper_ema)

        return {"pepper_prices": cleaned_prices, "pepper_ema": pepper_ema}
    ...
    def _next_ema(self, current_mid, previous_ema):
        if current_mid is None:
            return previous_ema
        if previous_ema is None:
            return float(current_mid)
        return (self.PEPPER_EMA_ALPHA * float(current_mid) + (1 - self.PEPPER_EMA_ALPHA) * float(previous_ema))
    ...
    def _take_mispriced(self, orders: List[Order], product: str, depth: OrderDepth, position: int, pending_buys: int, pending_sells: int) -> Tuple[int, int]:
    ...
    # ── Layer 1b: Flatten at fair value ────────────────────────────────
    def _flatten_at_fair(self, orders: List[Order], product: str, depth: OrderDepth, position: int, pending_buys: int, pending_sells: int) -> Tuple[int, int]:
        fair = self.FAIR_VALUE[product]
    ...
    # ── Layer 2: Penny-jump passive quotes ─────────────────────────────
    def _penny_jump_quotes(self, orders: List[Order], product: str, depth: OrderDepth, position: int, pending_buys: int, pending_sells: int) -> Tuple[int, int]:
        fair = self.FAIR_VALUE[product]
    ...
    def _emergency_flatten(self, orders: List[Order], product: str, depth: OrderDepth, position: int, pending_buys: int, pending_sells: int) -> Tuple[bool, int, int]:
        ...
        elif projected < -self.EMERGENCY_THRESHOLD[product]:
    ...
    def _trade_pepper_root(self, state: TradingState, history: list, current_ema) -> List[Order]:
        """Fast EMA-based mean-reversion with tiered sizing and inventory skew."""
        ...
        # Emergency flatten takes priority
        if position > self.PEPPER_EMERGENCY_THRESHOLD:
            ...
            self._place_sell(
                orders, product, emergency_price,
                flatten_qty, position, pending_sells,
            )
            return orders
        ...
        if position < -self.PEPPER_EMERGENCY_THRESHOLD:
            ...
            self._place_buy(
                orders, product, emergency_price,
                flatten_qty, position, pending_buys,
            )
            return orders

        # Warmup: passive quoting only until EMA has enough data
        if len(history) < self.PEPPER_WARMUP_TICKS:
            pending_buys = self._place_buy(
                orders, product,
                rounded_mid - self.PEPPER_WARMUP_DISTANCE,
                self.PEPPER_WARMUP_SIZE, position, pending_buys,
            )
            pending_sells = self._place_sell(
                orders, product,
                rounded_mid + self.PEPPER_WARMUP_DISTANCE,
                self.PEPPER_WARMUP_SIZE, position, pending_sells,
            )
            return orders

        if current_ema is None:
            current_ema = float(current_mid)

        signal = float(current_mid) - float(current_ema)

        # Inventory-adjusted signal thresholds
        adjusted_sell_threshold = (
            self.PEPPER_SIGNAL_THRESHOLD - position * self.PEPPER_INVENTORY_SKEW
        )
        adjusted_buy_threshold = -(
            self.PEPPER_SIGNAL_THRESHOLD + position * self.PEPPER_INVENTORY_SKEW
        )

        if signal <= adjusted_buy_threshold:
            # BUY signal — price is below EMA
            take_qty, quote_qty, inside_ticks = self._pepper_size_plan(
                max(abs(signal), float(self.PEPPER_SIGNAL_THRESHOLD))
            )
            max_buy_price = int(round(current_ema))
            pending_buys = self._take_asks(
                orders, product, depth, max_buy_price,
                position, pending_buys, max_total=take_qty,
            )

            aggressive_bid = self._inside_bid(bb, ba, inside_ticks, rounded_mid)
            aggressive_bid = min(aggressive_bid, max_buy_price)
            pending_buys = self._place_buy(
                orders, product, aggressive_bid,
                quote_qty, position, pending_buys,
            )

        elif signal >= adjusted_sell_threshold:
            # SELL signal — price is above EMA
            take_qty, quote_qty, inside_ticks = self._pepper_size_plan(
                max(abs(signal), float(self.PEPPER_SIGNAL_THRESHOLD))
            )
            min_sell_price = int(round(current_ema))
            pending_sells = self._take_bids(
                orders, product, depth, min_sell_price,
                position, pending_sells, max_total=take_qty,
            )

            aggressive_ask = self._inside_ask(bb, ba, inside_ticks, rounded_mid)
            aggressive_ask = max(aggressive_ask, min_sell_price)
            pending_sells = self._place_sell(
                orders, product, aggressive_ask,
                quote_qty, position, pending_sells,
            )

        else:
            # Neutral — small two-sided passive presence
            pending_buys = self._place_buy(
                orders, product,
                rounded_mid - self.PEPPER_PASSIVE_DISTANCE,
                self.PEPPER_PASSIVE_SIZE, position, pending_buys,
            )
            pending_sells = self._place_sell(
                orders, product,
                rounded_mid + self.PEPPER_PASSIVE_DISTANCE,
                self.PEPPER_PASSIVE_SIZE, position, pending_sells,
            )
    ...
    def run(self, state: TradingState):
        ...
        pepper_history = data.get("pepper_prices", [])
        previous_ema = data.get("pepper_ema")
        ...
        current_ema = self._next_ema(pepper_mid, previous_ema)
        ...
        if "INTARIAN_PEPPER_ROOT" in state.order_depths:
            result["INTARIAN_PEPPER_ROOT"] = self._trade_pepper_root(state, pepper_history, current_ema)

        if pepper_mid is not None:
            pepper_history = (pepper_history + [float(pepper_mid)])[-self.PEPPER_PRICE_HISTORY_LENGTH:]
        data["pepper_prices"] = pepper_history
        data["pepper_ema"] = current_ema
