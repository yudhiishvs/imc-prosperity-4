...
class Trader:
    ...
    MAF binary search log:
      maf1 (345109): bid=2000 → PnL=9290 (+102 vs 9188 baseline)
        - 47 missed sells (down from 83) — extra flow filling ask side
        - 3 whale hits all profitable (+90, +71, +85)
        - Improvement consistent with winning MAF auction
      maf2 (next): bid=4000 → confirm we're in winning half, then search down
        - If PnL improves further: winner with bid=4000, try 3000 next
        - If PnL same as baseline: bid=2000 was insufficient, try 6000 next
      Target: find minimum winning bid (maximize net value = extra_flow_value - MAF_cost)
    """

    MAF_BID = 4000   # XIRECs — maf2: testing higher bid to confirm winning half
