...
class Trader:
    ...
    MAF binary search log:
      ...
      maf1 (345109):      bid=2000  → PnL=9290  (+102)
      maf2 (345681):      bid=4000  → PnL=8993  (-195)
      maf3 (345925):      bid=3000  → PnL=9208  (+20)
      maf4 (346197):      bid=1000  → PnL=9055  (-133)

    Analysis — tests do NOT reflect MAF (testing always runs at 80% flow per rules).
    All PnL differences are pure run variance (±300). Binary search via test logs is
    invalid. bid=2000 had best observed test PnL (9290). Locking in bid=2000 for
    actual competition submission.

    v6 changes:
      - MAF_BID reverted to 2000 (best observed result)
      - Empty-book NaN guard in _trade_osmium (skip tick when both sides empty)
      - math.isnan guard on current_mid before EMA update
    """

    MAF_BID = 2000   # XIRECs — locked in, best observed test result
    ...
    def _mid_price(depth: OrderDepth):
        ...
        if best_bid is not None and best_ask is not None:
            ...
            return None if math.isnan(mid) else mid
        val = best_bid if best_bid is not None else best_ask
    ...
    def _trade_osmium(self, state: TradingState, data: dict) -> Tuple[List[Order], dict]:
        ...
        if not depth.buy_orders and not depth.sell_orders: return [], data  # empty book tick
        orders: List[Order] = []
        ...
        if current_mid is None or math.isnan(current_mid):
