...
class Trader:
    """
    v2 — based on 343468 (best baseline, PnL 9189) with 6 targeted fixes:

    FIX 1  PEPPER: reached_80 resets when pos drops below 70, so accumulation
           mode re-engages instead of crawling back at fair-2.
    FIX 2  PEPPER: MM inject never posts asks when deficit=0. At pos=80 we only
           maintain bids; the trend is up so selling early loses carry.
    FIX 3  PEPPER: Stale timer only triggers when position is BELOW target (stuck
           trying to buy). At pos=80 with no fills it means the market is happy —
           don't force a sell.
    FIX 4  PEPPER: Hard-close floor raised from 62 to 75. PEPPER is long-only
           in an uptrend; there's no settlement risk to justify selling 18 units
           1 tick before end-of-day.
    FIX 5  ASH: When already long and OIM is bullish, don't push ask away or cut
           ask size. Decouple bid/ask signal scaling by inventory direction.
    FIX 6  ASH: OIM taking was gated on ask < fv - edge, which almost never fires
           (ask ≈ fv + 8 in normal market). Replace with direct mispriced-take
           against constant fair=10000, matching the Roll-model EDA finding.
    """

    # ── ASH constants ─────────────────────────────────────────────────────────
    OSMIUM_POSITION_LIMIT       = 80
    OSMIUM_CONST_FAIR           = 10000    # FIX 6: known constant for mispriced-take
    OSMIUM_INITIAL_FV           = 10004    # EMA starting point
    ...
    OSMIUM_OIM_TAKE_SCALE       = 4.526    # kept but now used for scaling mispriced-take
    ...
    # FIX 4: End-of-day targets raised — PEPPER is long-only, no settlement risk
    ...
    PEPPER_HARD_CLOSE_TARGET    = 75    # was 62; raised to 75
    ...
    # FIX 1: threshold at which reached_80 resets (pos dropped well below limit)
    PEPPER_REACHED_80_RESET     = 70
    ...
    @staticmethod
    def _second_bid_ask(depth: OrderDepth) -> Tuple[Optional[int], Optional[int]]:
        bid2 = (sorted(depth.buy_orders.keys(), reverse=True)[1]
                if len(depth.buy_orders) >= 2 else None)
        ask2 = (sorted(depth.sell_orders.keys())[1]
                if len(depth.sell_orders) >= 2 else None)
        return bid2, ask2
    ...
    def _place_buy(self, orders, product, price, desired_qty, position, pending_buys) -> int:
        qty = min(desired_qty, self._buy_room(product, position, pending_buys))
        if qty > 0:
            orders.append(Order(product, price, qty))
            pending_buys += qty
        return pending_buys

    def _place_sell(self, orders, product, price, desired_qty, position, pending_sells) -> int:
        qty = min(desired_qty, self._sell_room(product, position, pending_sells))
        if qty > 0:
            orders.append(Order(product, price, -qty))
            pending_sells += qty
        return pending_sells

    def _take_asks(self, orders, product, depth, max_price, position, pending_buys, max_total=None) -> int:
        bought = 0
    ...
    def _take_bids(self, orders, product, depth, min_price, position, pending_sells, max_total=None) -> int:
        sold = 0
    ...
    def _trade_osmium(self, state: TradingState, data: dict) -> Tuple[List[Order], dict]:
        ...
        pending_buys = pending_sells = 0

        best_bid, best_ask = self._best_bid_ask(depth)
        current_mid = self._two_sided_mid(depth)
        osmium_ema = data.get("osmium_ema") or float(self.OSMIUM_INITIAL_FV)
        ...
        spread_now = (float(best_ask - best_bid) if best_bid > 0 and best_ask > 0
                      else data.get("osmium_spread_ema", 12.0))
        ...
        # FIX 6: Take mispriced liquidity against constant fair (10000), not EMA.
        # Original OIM-gated take almost never fired (ask ≈ fv+8 in normal market).
        # This layer fires whenever the market offers below/above 10000.
        for ask in sorted(depth.sell_orders):
            if ask >= self.OSMIUM_CONST_FAIR:
                break
            vol = -depth.sell_orders[ask]
            pending_buys = self._place_buy(orders, product, ask, vol, position, pending_buys)

        for bid in sorted(depth.buy_orders, reverse=True):
            if bid <= self.OSMIUM_CONST_FAIR:
                break
            vol = depth.buy_orders[bid]
            pending_sells = self._place_sell(orders, product, bid, vol, position, pending_sells)

        # Inventory-skewed quote sizing (unchanged from 343468)
        projected = position + pending_buys - pending_sells
        effective_pos_ratio = ((projected / self.OSMIUM_POSITION_LIMIT) +
                               (current_mid - osmium_fv) * self.OSMIUM_FV_TETHER_SCALE)
        ...
        # FIX 5: Decouple bid/ask OIM signal by inventory direction.
        # Original: bullish OIM always pushed ask up + faded ask size, even when long.
        # Now: if already long and OIM bullish → keep ask competitive (shift=0, scale=1).
        bid_shift, ask_shift, b_sig, a_sig = 0, 0, 1.0, 1.0
        if effective_oim > self.OSMIUM_OIM_THRESHOLD:
            bid_shift = self.OSMIUM_OIM_SHIFT
            b_sig     = self.OSMIUM_OIM_EDGE_SCALE
            if projected > 0:
                # Already long + bullish OIM: keep ask close and full-size to capture fills
                ask_shift = 0
                a_sig     = 1.0
            else:
                ask_shift = self.OSMIUM_OIM_SHIFT
                a_sig     = self.OSMIUM_OIM_FADE_SCALE
        elif effective_oim < -self.OSMIUM_OIM_THRESHOLD:
            ask_shift = -self.OSMIUM_OIM_SHIFT
            a_sig     = self.OSMIUM_OIM_EDGE_SCALE
            if projected < 0:
                # Already short + bearish OIM: keep bid close and full-size
                bid_shift = 0
                b_sig     = 1.0
            else:
                bid_shift = -self.OSMIUM_OIM_SHIFT
                b_sig     = self.OSMIUM_OIM_FADE_SCALE
        ...
        # Settlement flattening (unchanged)
        ts = state.timestamp
        ...
        # Quote placement with penny-jumping (unchanged)
        ...
        max_b = math.floor(osmium_fv) - 1 + bid_shift
        min_a = math.ceil(osmium_fv)  + 1 + ask_shift

        jump_b = jump_a = False
        if bid1 > 0 and bid1 + 1 <= max_b:
            mm_bid = max(mm_bid, bid1 + 1)
        elif bid2 is not None and bid2 + 1 <= max_b:
            mm_bid = max(mm_bid, bid2 + 1); jump_b = True

        if ask1 > 0 and ask1 - 1 >= min_a:
            mm_ask = min(mm_ask, ask1 - 1)
        elif ask2 is not None and ask2 - 1 >= min_a:
            mm_ask = min(mm_ask, ask2 - 1); jump_a = True
    ...
    def _pepper_target_position(self, state: TradingState, position: int,
                                resid_ema: float, data: dict) -> int:
        ts      = state.timestamp
        target  = self.PEPPER_POSITION_LIMIT

        # FIX 4: Hard close raised from 62 → 75 (was losing ~180 PnL by selling 18 units 1 tick from close)
        if ts >= self.PEPPER_HARD_CLOSE_TS:
            target = self.PEPPER_HARD_CLOSE_TARGET   # 75
        elif ts >= self.PEPPER_CLOSE_REDUCE_TS:
            target = self.PEPPER_CLOSE_TARGET         # 78
        elif ts >= self.PEPPER_LATE_REDUCE_TS:
            target = self.PEPPER_LATE_TARGET          # 80

        # FIX 3: Stale timer only fires when position is BELOW current target.
        # Being at pos=80 with no fills is the ideal state, not a reason to sell.
        stale_for = ts - data.get("pepper_last_pos_change_ts", 0)
        if stale_for > self.PEPPER_STALE_POS_TS and ts > 30000 and position < target:
            target = min(target, self.PEPPER_STALE_TARGET)

        # Residual-EMA defensive trim (unchanged — reacts to downward price breaks)
        if resid_ema is not None:
            if resid_ema <= self.PEPPER_STRONG_BREAK_RESID_EMA:
                target = min(target, self.PEPPER_STRONG_BREAK_TARGET)
            elif resid_ema <= self.PEPPER_SOFT_BREAK_RESID_EMA:
                target = min(target, self.PEPPER_SOFT_BREAK_TARGET)

        return int(target)

    def _trade_pepper_root(self, state: TradingState, base_estimate, resid_ema,
                           data: dict) -> Tuple[List[Order], dict]:
        ...
        # FIX 1: reached_80 resets when position drops well below limit.
        # Without this, after a defensive trim the strategy uses RECOUP_MAX_MARGIN=-2
        # (bids at fair-2) instead of the aggressive initial accumulation (fair+8).
        reached_80 = data.get("pepper_reached_80", False)
        if position >= self.PEPPER_POSITION_LIMIT:
            reached_80 = True
        elif position < self.PEPPER_REACHED_80_RESET:   # e.g. dropped to 69 or below
            reached_80 = False

        fair        = float(base_estimate or current_mid) + self.PEPPER_SLOPE * float(state.timestamp)
        fair_center = int(round(fair))
        target_pos  = self._pepper_target_position(state, position, resid_ema, data)
        pending_buys = pending_sells = 0

        # OIM shift (unchanged)
        bid_shift = ask_shift = 0
        best_bid, best_ask = self._best_bid_ask(depth)
        if best_bid > 0 and best_ask > 0:
            ...
            if vol > 0:
                ...
                if abs(oim) > self.PEPPER_OIM_BASE_THRESHOLD:
                    mag = min(self.PEPPER_OIM_MAX_SHIFT,
                              int((abs(oim) - 0.1) / 0.9 * self.PEPPER_OIM_MAX_SHIFT) + 1)
                    if oim < 0: bid_shift = -mag
                    else:       ask_shift =  mag

        # Defensive trim: sell if above target (unchanged)
        if position > target_pos:
            margin = (self.PEPPER_CLOSE_SELL_MARGIN
                      if state.timestamp > 99000 or (resid_ema and resid_ema < -2.5)
                      else self.PEPPER_SCALP_MIN_MARGIN)
            excess        = position - target_pos
            pending_sells = self._take_bids(orders, product, depth,
                                            fair_center + margin, position, pending_sells,
                                            max_total=excess)

        # FIX 2: MM inject — only post asks if there's a deficit (we need to reduce).
        # Original posted asks even at pos=80 (deficit=0), bleeding trend carry.
        # Now: at pos=80 we only bid to top up; asks only appear when deficit < 0
        # (i.e., a sell already happened and we're trying to come back while making spread).
        if reached_80 and position > 40:
            ...
            if (bid2 and ask2
                ...
                # Bids: always post to fill back to limit
                ...
                # Asks: ONLY post when deficit < 0 (oversold in prior turn and want to recoup spread)
                # At deficit=0 (pos=80) no asks — don't sell into the uptrend for zero reason
                if deficit < 0 and position >= self.PEPPER_MM_MIN_LONG_POSITION:
                    ask_qty       = max(0, self.PEPPER_MM_BASE_QUOTE_SIZE - bid_qty)
                    pending_sells = self._place_sell(orders, product, mm_ask, ask_qty,
                                                     position, pending_sells)

        # Basic accumulation (unchanged)
        remaining = target_pos - (position + pending_buys - pending_sells)
        if remaining > 0:
            if not reached_80:
                pending_buys = self._take_asks(orders, product, depth,
                                               fair_center + self.PEPPER_INITIAL_ACC_THRESH,
                                               position, pending_buys, max_total=remaining)
                remaining    = target_pos - (position + pending_buys - pending_sells)
                if remaining > 0:
                    pending_buys = self._place_buy(orders, product, fair_center,
                                                   remaining, position, pending_buys)
            else:
                pending_buys = self._place_buy(orders, product,
                                               fair_center + self.PEPPER_RECOUP_MAX_MARGIN,
                                               remaining, position, pending_buys)
    ...
    def run(self, state: TradingState):
        ...
        base       = self._pepper_base_estimate(pepper_mid, state.timestamp,
        ...
        data.update({"pepper_base_estimate": base, "pepper_resid_ema": resid,
                     "pepper_prev_mid": pepper_mid})
