...
class Trader:
    """
    v4 — 343468 baseline + MAF bid.

    MAF: bid() returns our Market Access Fee. Top 50% of bidders across all
    participants win extra flow (25% more quotes to trade against). The fee is
    subtracted from profit only if we win. Position limits stay at 80.

    MAF binary search log:
      baseline (343468):  bid=none  → PnL=9188
      maf1 (345109):      bid=2000  → PnL=9290  (+102 vs baseline)
      maf2 (?):           bid=4000  → PnL=8933  (-255 vs baseline)
      maf3 (next):        bid=3000  → binary midpoint between 2000 and 4000

    Interpretation of maf2 result:
      8933 < 9188 baseline. Two possibilities:
        A) Won at 4000, overpaid: flow_value = 8933 - 9188 + 4000 = 3745 XIREC
           but then bid=2000 winning would give 9188 + 3745 - 2000 = 10933 ≠ 9290 (inconsistent)
        B) Lost at both 2000 and 4000 (both within ±300 variance of baseline)
           threshold is above 4000 → but maf3=3000 would also lose
        C) Won at 2000 (flow≈102+2000=2102), lost at 4000 (just bad variance run)
           but losing at 4000 when 2000 wins makes no sense (higher bid always wins)

      Most likely: natural variance (±300/run) makes signal noisy. maf3=3000 narrows
      the range regardless. Decision rule for maf4:
        maf3 > baseline+100 → threshold < 3000, try 2500 next (minimize cost)
        maf3 ≈ baseline±200 → ambiguous, try 5000 next (test upper range)
        maf3 < baseline-200 → won at 3000 expensively, try 2500 next
    """

    MAF_BID = 3000   # XIRECs — maf3: binary midpoint 2000/4000
