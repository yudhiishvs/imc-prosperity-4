...
class Trader:
    ...
    with a tiny PEPPER passive ask overlay
    """
    ...
    # Tiny PEPPER MM overlay
    PEPPER_MM_OVERLAY_SIZE = 2
    PEPPER_MM_OVERLAY_MIN_MARGIN = 6
    PEPPER_MM_OVERLAY_MIN_POSITION = 70
    ...
    # PEPPER ROOT from #1 plus tiny passive ask overlay
    def _trade_pepper_root(self, state: TradingState, base_estimate, reached_80: bool) -> Tuple[List[Order], bool]:
        ...
        # Tiny passive ask overlay
        projected_position = position + pending_buys - pending_sells
        if reached_80 and projected_position >= 70:
            overlay_floor = fair_center + 6
            overlay_ask = None

            if best_ask > 0 and best_ask > overlay_floor:
                overlay_ask = max(best_ask - 1, overlay_floor)
            elif best_bid > 0:
                overlay_ask = max(best_bid + 1, overlay_floor)
            else:
                overlay_ask = overlay_floor

            if best_bid == 0 or overlay_ask > best_bid:
