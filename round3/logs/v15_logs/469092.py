"""
v15_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes vs v14
──────────────────────────────────────────────────────────────────────────────
1. HG TREND-REVERSAL CLOSE  — Biggest fix. Analysis showed HG PnL peaks at
   1563 (ts=91100) then drops 1030 to 533 because we hold short=-23 through
   the entire price recovery (EMA hasn't caught up). Fix: persist
   hg_in_downtrend across ticks. When it flips True→False with pos<0,
   immediately issue a taker BUY at ba to close the short — locking in peak
   PnL instead of riding the reversal.

2. VF DUAL-EMA TREND FILTER  — VF shows clear directional periods: down
   at tick 500–700, up at tick 900. Without a filter we passively buy VF
   throughout the downtrend and build a losing long. Fix: track fast EMA
   (α=0.10) and slow trend EMA (α=0.005) on VF mid. When VF is in downtrend
   (fast < trend - gap), suppress bids. When in uptrend (fast > trend + gap),
   suppress asks. This prevents accumulating directional exposure against
   the trend while still quoting in flat/slow markets.

All other logic unchanged from v14 (IV smile, vol signal, EOD close, etc.).
"""
...
class Trader:
    ...
    # VF dual-EMA trend filter (NEW v15)
    # Suppress bids when VF trending down, suppress asks when trending up.
    VF_EMA_ALPHA   = 0.10
    VF_TREND_ALPHA = 0.005
    VF_TREND_GAP   = 3
    ...
    def _load(self, raw: str) -> Dict[str, Any]:
        default: Dict[str, Any] = {
            ...
            "hg_vol": None, "hg_in_downtrend": False,
            "vf_ema": None, "vf_trend_ema": None,
            "day": 0, "prev_ts": -1,
        }
        if not raw:
            return default
    ...
    def _trade_hg(self, state: TradingState, data: Dict) -> Tuple[List[Order], Dict]:
        ...
        # Trend-reversal close (NEW v15):
        # When downtrend ends (was down last tick, flat/up this tick) and we
        # hold a short, buy immediately at market to lock in the PnL peak.
        prev_in_downtrend = data["hg_in_downtrend"]
        data["hg_in_downtrend"] = in_downtrend
        reversal = prev_in_downtrend and not in_downtrend

        if reversal and pos < 0:
            qty = min(-pos, bav, self._buy_room(pos, 0, self.HG_LIMIT))
            if qty > 0:
                orders.append(Order(self.HG, ba, qty))
                logger.print(
                    f"[HG REVERSAL CLOSE] pos={pos} qty={qty}@{ba}"
                    f" ema={ema:.1f} trend={trend_ema:.1f}"
                )
            return orders, data
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # 2. VF PASSIVE MM  (+ dual-EMA trend filter, NEW v15)
    # ─────────────────────────────────────────────────────────────────────────

    def _trade_vf(self, state: TradingState, data: Dict) -> Tuple[List[Order], Dict]:
        """
        Passive MM on VELVETFRUIT_EXTRACT with dual-EMA trend filter.

        Trend filter (NEW v15): track fast EMA (α=0.10) and slow trend EMA
        (α=0.005) on VF mid. When fast < trend - gap (downtrend), suppress
        bids to avoid building long exposure into a falling market. When fast
        > trend + gap (uptrend), suppress asks to avoid shorting into a rally.
        In flat/slow markets, quote both sides as before.
        """
        ...
        # Update VF EMAs for trend filter
        vf_mid = (bb + ba) / 2.0
        vf_ema   = self._ewma(data["vf_ema"],       vf_mid, self.VF_EMA_ALPHA)
        vf_trend = self._ewma(data["vf_trend_ema"], vf_mid, self.VF_TREND_ALPHA)
        data["vf_ema"]       = vf_ema
        data["vf_trend_ema"] = vf_trend

        vf_fast_vs_trend = vf_ema - vf_trend
        vf_in_downtrend  = vf_fast_vs_trend < -self.VF_TREND_GAP
        vf_in_uptrend    = vf_fast_vs_trend >  self.VF_TREND_GAP
        ...
        trend_str = "DOWN" if vf_in_downtrend else ("UP" if vf_in_uptrend else "flat")

        if pos >= self.VF_MM_SOFT_CAP:
            # Skewed short: only sell
            ...
            # Skewed long: only buy
            bq = min(self.VF_QUOTE_SIZE, self._buy_room(pos, 0, self.VF_MM_POS_CAP))
            if bq > 0:
                orders.append(Order(self.VF, bid_p, bq))
        else:
            # Two-sided with trend filter
            bq = min(self.VF_QUOTE_SIZE, self._buy_room(pos, 0, self.VF_MM_POS_CAP))
            aq = min(self.VF_QUOTE_SIZE, self._sell_room(pos, 0, self.VF_MM_POS_CAP))

            # Suppress bids during VF downtrend (avoid accumulating long)
            if vf_in_downtrend:
                bq = 0
            # Suppress asks during VF uptrend (avoid accumulating short)
            if vf_in_uptrend:
                aq = 0
        ...
        if orders:
            logger.print(
                f"[VF] pos={pos} spread={spread} trend={trend_str}"
                f"({vf_fast_vs_trend:+.2f}) bid={bid_p} ask={ask_p}"
            )
        return orders, data
    ...
    def run(self, state: TradingState):
        ...
        # 1. HG: dual-EMA + trend-reversal close + vol signal + EOD close
        if self.HG in state.order_depths:
            hg_ords, data = self._trade_hg(state, data)
            result[self.HG] = hg_ords

        # 2. VF: passive MM + dual-EMA trend filter + EOD close
        vf_mm_ords, data = self._trade_vf(state, data)
