"""
v13_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes vs v12
──────────────────────────────────────────────────────────────────────────────
1. VF SCALE-UP  — VF passive MM was +585 PnL in v12 (best single product).
   Size 15→25, pos cap 100→150. Expected: +50-60% more VF PnL.
   Also added VF EOD close (ts>950000) to avoid overnight directional exposure.

2. SPREAD=2 FIX  — K=5300 and K=5400 have spread=2 → we can't quote both
   sides inside (bb+1 == ba-1 at same price → crossed). Old code falls back
   to market prices (bb/ba), meaning we're behind bots in queue → 0 own fills.
   Fix: for spread==2, post bid@bb+1 (inside priority on sell flow) and
   ask@ba (at market). We win the bid queue; sellers hitting the bid choose
   us first over bots at bb.

3. ITM CAPS INCREASE  — K=4500 has 16-tick spread (best edge/fill in options).
   v12 cap=25 limits p_size=12, getting only 5 fills. Raise to 50/25.

4. LARGER OPTION QUOTES  — OPT_PASSIVE_SIZE 15→25. More capture per bot event
   (bot trades 5000/5100/5200 simultaneously, so bigger size = more captured).

5. LIVE IV SMILE + VOL-SKEWED SIZING  — Orin's insight: "Plot IV vs moneyness.
   Find the odd ones out." Per tick we compute live IV (bisection, stable) for
   near-ATM strikes (5000–5400), fit a linear smile, and compute residuals.
   - Strike with IV > smile (expensive vol): increase ask size, decrease bid size
   - Strike with IV < smile (cheap vol):     increase bid size, decrease ask size
   Vol signal multiplier: 1.5× on the favored side, 0.75× on the adverse side.
   Deep ITM (4000/4500) excluded from smile fit (bid < intrinsic → distorted IV).

HG: unchanged dual-EMA + maker guard + EOD close + vol signal (all from v12).
...
class Trader:
    ...
    def _trade_options(
        ...
        Spread handling:
          ...
          spread == 2: bid@bb+1, ask@ba        (inside bid priority, ask at market)
          spread == 1: skip (no edge possible)
        ...
        for K in self.ACTIVE_STRIKES:
            ...
            # Skip 1-tick spreads (no room to earn any edge)
            if spread < 2:
                continue
            ...
            # ── Quote prices based on spread ──────────────────────────────────
            if spread >= 3:
                bid_p = bb + 1
                ask_p = ba - 1
            else:
                # spread == 2: inside priority only on bid side.
                # bb+1 = ba-1 (same price) → can't do both inside without
                # crossing our own orders. Post bid inside, ask at market.
                bid_p = bb + 1   # inside: gets priority over bots at bb
                ask_p = ba       # at market: queue competition but still valid
