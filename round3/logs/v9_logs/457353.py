"""
v9_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes vs v8_round3_2
───────────────────────
1. OPTIONS: neutral MM quotes at bb+1 / ba-1 (inside market) for queue priority.
   v8 posted at bb/ba = behind existing 20-unit market bots → zero fills.
   v9 posts 1 tick inside the spread (when ba-bb ≥ 3) to be first in queue.

2. HG: faster EMA (0.020→0.050) + larger taker edge (8→12).
   v8 EMA lagged ~50 ticks during downtrend → TAKE BUY fired 99× into falling
   market.  Faster alpha reduces lag to ~20 ticks so the gap to EMA collapses
   during trends, greatly reducing false buy signals.

Options (unchanged from v8)
──────────────────────────────
• PRE-CALIBRATED BID/ASK SMILE — used ONLY for TTE inference at startup.
...
• OPT_POS_CAP = 150, OPT_SOFT_CAP = 60.
• Deep-ITM strikes (4000, 4500) included in delta computation if held,
  but NOT actively MM'd.
• Delta hedge DEADBAND = 20.
...
class Trader:
    ...
    HG_EMA_ALPHA   = 0.050   # faster tracking — reduces lag in trending markets
    HG_TAKE_EDGE   = 12      # larger edge to suppress false signals during trends
    HG_TAKER_SIZE  = 10
    HG_QUOTE_TICK  = 2
    HG_SKEW_TICKS  = 3.0
    HG_QUOTE_SIZE  = 20
