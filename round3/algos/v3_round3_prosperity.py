"""
v3_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes from v2:
  VEV options — online parabola smile fit (Hedgehog approach):
    • At every tick, compute IV for the 6 ATM-band strikes (5000–5500).
    • Fit quadratic IV(log-moneyness) via pure-Python 3×3 Cramer's rule.
    • Trade when |market_IV − fitted_IV| > VEV_SMILE_THRESHOLD (0.008).
      Direction is confirmed against EDA priors so we only trade when online
      signal and pre-computed signal agree.
    • Replaced fixed-σ BS scalp entirely.

  VEV_6000 / VEV_6500 — keep passive short at ask=1:
    • EDA residual for VEV_6500 is technically negative (market IV < smile) but
      the floor-price mechanics make IV computation unreliable at that moneyness.
    • P(S>6500 in 5 days | S≈5250, σ≈22%) ≈ 0.0006% → selling at 1 is ~free money.

  VEV_4000 / VEV_4500 — still skipped:
    • Residuals are large but both options are deep ITM → time value ≈ 0.
    • Spread (20 ticks) >> IV-discount edge (~0 ticks). Not tradeable.

  HYDROGEL — unchanged from v2 (EMA MM + taker).

  VF Extract — unchanged from v2 (EMA MM + imbalance skew, limit=200).

  All position limits confirmed from official docs:
  ...
  TTE mapping (live Round 3 execution):
    ...
    day_counter increments on timestamp reset (new game-day within submission).

EDA residual priors (Day 2 = TTE≈4.5, closest to live conditions):
  strike 5000 → residual −0.016 (BUY — underpriced)
  strike 5100 → residual −0.010 (BUY)
  strike 5200 → residual +0.003 (SELL — overpriced)
  strike 5300 → residual +0.007 (SELL)
  strike 5400 → residual −0.015 (BUY)
  strike 5500 → residual −0.007 (BUY)
"""
...
class Trader:
    ...
    # Used to confirm the online signal; if signs disagree we skip.
    ...
    # Max positions per scalp strike (scaled from conviction map, well inside limit=300)
    ...
    VEV_SMILE_EXIT       = 0.003   # |δIV| to exit (residual has converged)
    ...
    # ── HYDROGEL parameters (unchanged from v1/v2) ───────────────────────────
    HG_EMA_ALPHA          = 0.005
    ...
    # ── VF Extract parameters (confirmed limit=200) ───────────────────────────
    VF_EMA_ALPHA            = 0.010
    ...
    def _fit_parabola(pairs: List[Tuple[float, float]]) -> Optional[Tuple[float, float, float]]:
        """
        Least-squares quadratic fit to (x, y) pairs.
        Returns (a, b, c) or None if system is singular / too few points.
        """
        if len(pairs) < 3:
            return None
        ...
        # 3×3 normal-equation matrix M and RHS rhs
        M = [
            [sx4, sx3, sx2],
            [sx3, sx2, sx1],
            [sx2, sx1,  s1],
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Wall helpers (unchanged from v1/v2 — low impact per EDA but harmless)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # HYDROGEL strategy (unchanged from v2)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # VF Extract strategy (unchanged from v2 except limit=200 already set)
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # VEV_6000 / VEV_6500 — passive short at floor price (unchanged from v2)
    # EDA: P(S>6000)≈0.3%, P(S>6500)≈0.0006% at TTE=5 — selling at 1 is
    # near-free premium.  Residual signal for 6500 is unreliable at floor price.
    # ─────────────────────────────────────────────────────────────────────────
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # VEV ATM scalp — online parabola smile fit (NEW in v3)
    #
    # Algorithm:
    #   1. Get current VF Extract mid price S.
    #   2. For each ATM-band strike (5000–5500):
    #        a. Get mid price of the option.
    #        b. Compute implied vol via Newton-Raphson.
    #        c. Collect (log(S/K), IV) pair if valid.
    #   3. Fit quadratic IV(x) to collected pairs (need ≥ 3).
    #   4. For each strike, compute residual = market_IV − fitted_IV.
    #   5. Trade when:
    #        • |residual| > VEV_SMILE_THRESHOLD, AND
    #        • online residual sign matches EDA prior direction for that strike.
    #      This two-signal confirmation prevents trading noise.
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _trade_vev_scalp(
        ...
        # Step 2: collect IV data points
        iv_pairs:  List[Tuple[float, float]]          = []   # (log_moneyness, iv)
        book_data: Dict[int, Tuple[int, int, int, int]] = {}  # strike → (bb,bbv,ba,bav)
        iv_data:   Dict[int, float]                    = {}  # strike → market_iv
        ...
        # Step 4 + 5: compute residuals and place orders
        for strike in self.VEV_SMILE_STRIKES:
            ...
            # SELL when market IV > smile AND prior says SELL (positive residual expected)
            ...
            # BUY when market IV < smile AND prior says BUY (negative residual expected)
            ...
            # EXIT when residual has converged back toward 0
            elif abs(residual) < self.VEV_SMILE_EXIT:
                if pos > 0:
                    # We're long → sell to flatten
                    qty = min(pos, self.VEV_SCALP_SIZE, bbv)
                    if qty > 0:
                        orders.append(Order(sym, bb, -qty))
                elif pos < 0:
                    # We're short → buy to flatten
    ...
    def run(self, state: TradingState):
        ...
        # 4. VEV_5000–5500: online smile-fit residual scalp
        for sym, orders in self._trade_vev_scalp(state, tte).items():
            result.setdefault(sym, []).extend(orders)

        # Ensure explicit empty list for every VEV product (good practice)
