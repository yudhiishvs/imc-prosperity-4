...
2. PASSIVE MARKET MAKING — quote at market bid/ask, earn the spread
   Pre-calibrated smile is used ONLY for TTE inference at startup.
   Quote prices = market bb / ba (not stale fair values).
   Position skewing: if short-heavy → improve bid to close; if long-heavy → improve ask.
...
class Trader:
    ...
    def _trade_options(
        ...
        Quote logic (position-skewed):
          |pos| ≤ SOFT_CAP  → quote both bid@bb and ask@ba (neutral MM)
        ...
        for K in self.ACTIVE_STRIKES:
            ...
            if pos <= -self.OPT_SOFT_CAP:
                ...
                # Neutral: quote both sides at market, earn the spread
                bq = min(self.OPT_PASSIVE_SIZE,
                         self._buy_room(pos, 0, self.OPT_POS_CAP))
                aq = min(self.OPT_PASSIVE_SIZE,
                         self._sell_room(pos, 0, self.OPT_POS_CAP))
