"""
v11_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes vs v9 (back to v9 base with two targeted HG fixes)
────────────────────────────────────────────────────────────
Plot analysis of v9 shows: red BUY fills cluster all along the HG downtrend
(10011→9927).  Two root causes:

1. SPREAD-CROSSING BUG: bid_p = EMA-2 can be > ba during sharp drops (EMA
   lags) → exchange treats it as a market buy → buys into every dip.
   Fix: Maker-only guard  bid_p = min(bid_p, ba-1) / ask_p = max(ask_p, bb+1)

2. NO TREND AWARENESS: we quote both bid AND ask symmetrically even during a
   sustained downtrend → buy every dip, sell at lower prices.
   Fix: Two-EMA trend filter
     fast_ema  α=0.050  (tracks recent price, window ~20 ticks)
     trend_ema α=0.003  (long-run mean, window ~330 ticks)
   When fast < trend - HG_TREND_GAP  → downtrend → bq = 0 (stop buying)
   When fast > trend + HG_TREND_GAP  → uptrend   → aq = 0 (stop selling)
   During downtrend we still quote asks, earning spread from buyers who come in.

3. SOFT_LIMIT lowered 140→60: max 60 units accumulated before inventory skew
   forces aggressive close.  Limits MTM exposure on open position.

Options: unchanged from v9 (working, +41 PnL in v9 test).
  Neutral MM at bb+1/ba-1 for queue priority.
  VEV_5000/5100/5200 getting fills.
...
class Trader:
    ...
    # All actively MM'd strikes (inside-market bid/ask quotes, earn spread).
    # Deep ITM (4000/4500) have 16-21 tick spreads — highest edge per fill.
    # Near-ATM (5000-5500) have 2-6 tick spreads.
    ACTIVE_STRIKES   = [4000, 4500, 5000, 5100, 5200, 5300, 5400, 5500]
    # Deep OTM — passive short at floor price 1
    FLOOR_STRIKES    = [6000, 6500]
    DEEP_ITM_STRIKES: list = []   # now part of ACTIVE_STRIKES
    ...
    # Near-ATM caps  (delta avg ~0.45, 6 strikes × 60 × 0.45 ≈ 162 delta → fits VF_LIMIT)
    OPT_POS_CAP   = 150   # hard cap: emergency market exit
    OPT_SOFT_CAP  = 60    # soft cap: start skewing quotes
    # Deep ITM caps  (delta ~0.95-1.0 — smaller cap to limit VF hedge usage)
    ITM_POS_CAP   = 25    # hard cap per ITM strike: 25 × 0.97 = ~24 delta each
    ITM_SOFT_CAP  = 12    # soft cap per ITM strike
    FLOOR_POS_CAP = 200
    ...
    # ── HG MM parameters ──────────────────────────────────────────────────────
    HG_EMA_ALPHA   = 0.050   # fast EMA for fair value / quote centre
    HG_TREND_ALPHA = 0.003   # slow EMA for trend direction (window ~330 ticks)
    HG_TREND_GAP   = 8       # ticks: |fast-trend| before directional bias kicks in
    HG_TAKE_EDGE   = 20      # taker threshold (rarely fires — safety valve only)
    HG_TAKER_SIZE  = 8
    HG_QUOTE_TICK  = 2       # ticks from fair on each side (before maker clamp)
    HG_SKEW_TICKS  = 5.0     # inventory skew ticks per full position
    HG_QUOTE_SIZE  = 15
    HG_SOFT_LIMIT  = 60      # stop bidding / start skewing earlier (was 140)
    HG_HARD_LIMIT  = 190
    HG_UNWIND_SIZE = 30
    ...
    def _trade_hg(self, state: TradingState, data: Dict) -> Tuple[List[Order], Dict]:
        ...
        # Trend direction from dual-EMA crossover
        fast_vs_trend = ema - trend_ema   # positive → uptrend, negative → downtrend
        ...
        if bb >= ema + self.HG_TAKE_EDGE:
            ...
            if qty > 0:
                orders.append(Order(self.HG, bb, -qty))
                logger.print(f"[HG TAKE SELL] bb={bb} ema={ema:.1f} trend={trend_ema:.1f}")
            return orders, data
        if ba <= ema - self.HG_TAKE_EDGE:
            qty = min(self.HG_TAKER_SIZE, bav, self._buy_room(pos, 0, self.HG_LIMIT))
            if qty > 0:
                orders.append(Order(self.HG, ba, qty))
                logger.print(f"[HG TAKE BUY] ba={ba} ema={ema:.1f} trend={trend_ema:.1f}")
            return orders, data
        ...
        # Without this, when EMA > market (lag during drops), bid_p > ba and the
        # exchange immediately fills it as a market buy — inadvertent taker.
        bid_p = min(bid_p, ba - 1)
        ask_p = max(ask_p, bb + 1)

        # ── Two-EMA trend filter: stop quoting the adverse side in a trend ────
        # In a downtrend: stop bidding (don't catch falling knives).
        #                 Still quote asks → earn spread from buyers who come in.
        # In an uptrend:  stop asking (don't sell into a rising market).
        #                 Still quote bids.
        base_bq = 0 if pos >= self.HG_SOFT_LIMIT else min(
            self.HG_QUOTE_SIZE, self._buy_room(pos, 0, self.HG_LIMIT))
        base_aq = 0 if pos <= -self.HG_SOFT_LIMIT else min(
            self.HG_QUOTE_SIZE, self._sell_room(pos, 0, self.HG_LIMIT))
        ...
        if pos >= self.HG_SOFT_LIMIT:
            ...
            ask_p = min(ask_p, bb + 1)  # aggressive ask to close long
        if pos <= -self.HG_SOFT_LIMIT:
            bq = min(self.HG_QUOTE_SIZE, self._buy_room(pos, 0, self.HG_LIMIT))
            bid_p = max(bid_p, ba - 1)  # aggressive bid to close short
    ...
    def _trade_options(
        ...
        for K in self.ACTIVE_STRIKES:
            ...
            # Per-strike caps: deep ITM (delta~1) uses tighter caps to preserve
            # VF hedge capacity; near-ATM uses standard caps.
            is_itm  = K <= 4500
            ...
            if pos <= -soft_cap:
                ...
                # Neutral: quote inside spread for queue priority (spread ≥ 3)
                if ba - bb >= 3:
                    bid_p = bb + 1
                    ask_p = ba - 1
                bq = min(p_size, self._buy_room(pos, 0, pos_cap))
            ...
            if orders:
                ...
                logger.print(
                    f"[OPT] {sym} pos={pos} spread={ba-bb} bid={bid_p}×{bq} ask={ask_p}×{aq} iv={iv_str}"
                )

        # ── IV for any floor-strike positions we hold (for delta) ─────────────
        for K in self.FLOOR_STRIKES:
