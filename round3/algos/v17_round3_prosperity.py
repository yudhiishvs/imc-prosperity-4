"""
v17_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes vs v16 (two parameter tweaks only — all logic identical)
──────────────────────────────────────────────────────────────────────────────
1. HG_SOFT_LIMIT  60 → 30
   Root cause found: in v16 the HG trend signal stays in DOWNTREND for all 89
   ticks after the price bottoms (ts=91100→99900). With soft_limit=60, pos=-36
   never triggers the soft cap, so base_aq>0 throughout → we keep adding
   shorts while bq=0 (no bids). Position stays frozen at -36, causing a 1620
   unrealized loss on the 45-tick recovery.
   With soft_limit=30: once pos ≤ -30, base_aq=0 (stops adding shorts) AND
   the soft-cap override forces bq>0 (posts bids) even during downtrend.
   Result: position converges to -30 in ~4 ticks, then oscillates around -30
   earning 14 ticks per round-trip while keeping max reversal exposure at 30×45
   = 1350 vs 1620. Expected improvement: ~+550 HG PnL.

2. HG_TREND_ALPHA  0.003 → 0.005
   Slightly faster trend EMA helps it converge to the recovery price sooner,
   exiting downtrend mode a few ticks earlier and allowing bids to be posted
   without relying solely on the soft-cap override.

All VF, options, bb+1/ba-1 quoting, and IV smile logic identical to v16.
"""
...
class Trader:
    ...
    # Fitted from 3-day market data IV = a*m^2 + b*m + c, m = log(S/K)/sqrt(T).
    # Pipe-shape surface: high IV at deep-ITM (K=4500, bowl), flat ~1.33% ATM/OTM (stem).
    # Old params underestimated bid IV at K=4500 by 1.4% (major TTE inference error).
    ASK_SMILE: Tuple[float, float, float] = (5.3206,  0.0250, 0.01341)
    BID_SMILE: Tuple[float, float, float] = (5.3206, -0.1176, 0.01266)
    ...
    def _trade_hg(self, state: TradingState, data: Dict) -> Tuple[List[Order], Dict]:
        ...
        # Inside-market quoting: price priority over bots at bb/ba.
        # HG spread is consistently 16 ticks → captures 14 ticks per round-trip
        # vs only 4 ticks with the old EMA±2 approach.
        if ba - bb < 3:
            return orders, data
        bid_p = bb + 1
        ask_p = ba - 1
