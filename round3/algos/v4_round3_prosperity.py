...
Changes from v3:
  ...
  2. VF drift compensation — REMOVED (bug fix)
       • The explicit drift offset (VF_DRIFT_PER_DAY * intraday_frac) pushed fair above
         raw_mid persistently, triggering the stress-based aq=0 path. Combined with
         bq=0 at soft limit, this created a dead zone with no trades and growing inventory.
       • EMA (α=0.010) already tracks the +8 tick/day trend. VF_DRIFT_PER_DAY = 0.0.

  2b. Forced unwind override (HG + VF) — NEW bug fix
       • When pos >= SOFT_LIMIT and aq was suppressed (by OIM or stress), override to
         force a sell quote at bb+1 (inside spread, guaranteed to attract fills).
       • Symmetric for short side. Prevents the stuck-inventory dead zone entirely.
...
class Trader:
    ...
    # v4: insider signal nudge for HG — reduced to 0.5 to avoid inflating fair above market
    HG_INSIDER_NUDGE_TICKS   = 0.5
    ...
    # v4: insider signal nudge for VF — reduced to 1 tick to avoid inflating fair above market
    VF_INSIDER_NUDGE_TICKS  = 1.0

    # v5 FIX: drift offset removed — EMA (α=0.010) already tracks the +8 tick/day drift.
    # Adding an explicit offset on top double-counted and pushed fair persistently above
    # raw_mid, which triggered the stress→aq=0 path and caused us to accumulate inventory
    # without ever selling. Set to 0.0 to disable.
    VF_DRIFT_PER_DAY        = 0.0
    ...
    def _trade_hydrogel(
        ...
        # Forced unwind: when at soft limit, bypass OIM/stress suppressions and quote
        # aggressively to clear inventory. This prevents the dead zone where bq=0 and
        # aq=0 simultaneously (stress/OIM suppressed sells while soft limit stopped buys).
        if pos >= self.HG_SOFT_POSITION_LIMIT and aq == 0:
            aq = min(qsize, self._sell_room(pos, ps, self.HG_POSITION_LIMIT))
            ask_p = min(ask_p, bb + 1)   # inside spread — will get filled
        elif pos <= -self.HG_SOFT_POSITION_LIMIT and bq == 0:
            bq = min(qsize, self._buy_room(pos, pb, self.HG_POSITION_LIMIT))
            bid_p = max(bid_p, ba - 1)   # inside spread — will get filled
    ...
    def _trade_vf_extract(
        ...
        # Forced unwind: same logic as HG — bypass suppressions when stuck at soft limit
        if pos >= self.VF_SOFT_LIMIT and aq == 0:
            aq = min(self.VF_BASE_QUOTE_SIZE, self._sell_room(pos, ps, self.VF_POSITION_LIMIT))
            ask_p = min(ask_p, bb + 1)
        elif pos <= -self.VF_SOFT_LIMIT and bq == 0:
            bq = min(self.VF_BASE_QUOTE_SIZE, self._buy_room(pos, pb, self.VF_POSITION_LIMIT))
