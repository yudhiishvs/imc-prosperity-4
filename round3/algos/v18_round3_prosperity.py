"""
v18_round3_prosperity.py
IMC Prosperity 4 — Round 3

Changes vs v17 (smile params only — all logic identical)
──────────────────────────────────────────────────────────────────────────────
ASK_SMILE / BID_SMILE  →  a=b=c=0.1

Fits the pipe: flat constant IV ≈ 0.1 across all moneyness levels.
"""
...
class Trader:
    ...
    ASK_SMILE: Tuple[float, float, float] = (0.2, 0.2, 0.1)
    BID_SMILE: Tuple[float, float, float] = (0.1, 0.1, 0.1)
    ...
    # ── Model-fair taker (options) ────────────────────────────────────────────
    # Lift the offer when market ask < BID_SMILE fair value by this margin.
    # Hit the bid  when market bid > ASK_SMILE fair value by this margin.
    OPT_TAKER_EDGE = 0.5
    OPT_TAKER_SIZE = 10
    ...
    def _trade_options(
        ...
        for K in self.ACTIVE_STRIKES:
            ...
            # Skip 0/1-tick spreads — no room to quote inside
            if spread < 2:
                continue
            ...
            # ── Taker: lift cheap offer / hit rich bid ────────────────────────
            ...
            # ── Passive: only quote sides the model supports ──────────────────
            # Model says option is worth at least bb → bid is sensible.
            # Model says option fair sell is at most ba → ask is sensible.
            can_bid = fair_bid >= float(bb)
            can_ask = fair_ask <= float(ba)

            # When spread=2, bb+1==ba-1: post only one side to avoid crossing
            if spread == 2:
                # Lean toward the side our inventory needs to unwind
                bid_p = bb + 1
                ask_p = ba - 1  # same price — only post one
                if pos >= 0:
                    bq = 0  # already long or flat: only offer
                else:
                    aq = 0  # already short: only bid
            bid_p = bb + 1
            ask_p = ba - 1
