...
class Trader:
    """
    v40 — Increase AS_GAMMA from 0.025 → 0.1 for faster ASH inventory turnover.
    Change from v39: AS_GAMMA=0.1 → ±8 tick shift at max position ±80 (= half the spread).
    Effect: at position +40 bids drop 2 ticks, at +60 bids drop 4 ticks, at +80 bids drop 6 ticks.
    Asks unaffected when long (capped below by reservation, but penny-jump ask stays above reservation).
    Net result: more directional pressure to sell when long, reducing time at extreme positions.
    """
    ...
    AS_GAMMA = {"ASH_COATED_OSMIUM": 0.1}
