"""
INTARIAN PEPPER ROOT Market-Making Analysis — IMC Prosperity 4
============================================================
...
OUT_DIR = pathlib.Path(__file__).resolve().parent / "pepper_root_analysis"
...
def load_prices() -> pd.DataFrame:
    """Load all price snapshot CSVs, filter to INTARIAN_PEPPER_ROOT."""
    frames = []
    ...
    df_filtered = all_df[all_df["product"] == "INTARIAN_PEPPER_ROOT"].copy()
...
def load_trades() -> pd.DataFrame:
    """Load all trade CSVs, filter to INTARIAN_PEPPER_ROOT."""
    frames = []
    ...
    if not frames:
        return pd.DataFrame()
    all_df = pd.concat(frames, ignore_index=True)
    df_filtered = all_df[all_df["symbol"] == "INTARIAN_PEPPER_ROOT"].copy()
...
def analysis_mid_price(df: pd.DataFrame):
    ...
    ax.set_title("INTARIAN_PEPPER_ROOT — Mid-Price Frequency Distribution", fontsize=13, fontweight="bold")
    # Hide overly dense x labels if too many
    if len(freq) > 40:
        ax.set_xticks([])
...
def analysis_spread(df: pd.DataFrame):
    ...
    ax.set_ylabel("Frequency")
    ax.set_title("INTARIAN_PEPPER_ROOT — Bid-Ask Spread Distribution", fontsize=13, fontweight="bold")
    ax.grid(axis="y", alpha=0.3)
    fig.tight_layout()
...
def analysis_timeseries(df: pd.DataFrame):
    ...
    axes[0].set_title("INTARIAN_PEPPER_ROOT — Price & Book Dynamics (Day 1)", fontsize=13, fontweight="bold")
...
def main():
    print("\n" + "█" * 72)
    print("  INTARIAN_PEPPER_ROOT MARKET-MAKING ANALYSIS")
    print("█" * 72)
