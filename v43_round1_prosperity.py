...
class Trader:
    """
    v43 — ASH directional mode.
    When |position| > DIRECTIONAL_THRESHOLD (40):
      - Hard-stop passive quotes on the adding side (vs volume skew soft cutoff at ~53)
      - Suppress _take_mispriced on the adding side (don't buy asks<=fair when already long 40+)
      - Tighten inner unwind quote by 1 tick for faster inventory mean-reversion
    Rationale: analysis showed ASH in elevated inventory (|pos| 30-60) 35-45% of time.
    Volume skew only kills bids at pos 53+; directional mode hard-stops at 40.
    """
    ...
    DIRECTIONAL_THRESHOLD = {"ASH_COATED_OSMIUM": 40}
    ...
    def _penny_jump_quotes(
        ...
        directional_long: bool = False,
        directional_short: bool = False,
    ) -> Tuple[int, int]:
        ...
        # Directional mode: hard-stop the adding side, tighten the unwind side by 1 tick
        if directional_long:
            total_bid_qty = 0
        ...
        # In directional_long mode, tighten ask by 1 tick to get filled faster
        if directional_long:
        ...
        # Mirror: tighten bid in directional_short
    ...
    def _trade_osmium(self, state: TradingState, fair_value, current_mid, last_mid) -> List[Order]:
        ...
        dt = self.DIRECTIONAL_THRESHOLD.get(product, 999)
        directional_long  = position >  dt
        directional_short = position < -dt

        # _take_mispriced: in directional mode, suppress the side that adds exposure
        pending_buys, pending_sells = self._take_mispriced(
            ...
            pending_sells,
            fair_value=fair,
            buy_inclusive=not directional_long,   # don't buy if already heavily long
            sell_inclusive=not directional_short,  # don't sell if already heavily short
        )
