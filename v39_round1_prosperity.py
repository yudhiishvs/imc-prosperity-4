...
class Trader:
    """
    v39 — Avellaneda-Stoikov reservation price for ASH quote placement.
    Change from v35: _penny_jump_quotes now centers on reservation_price = fair - q * AS_GAMMA
    instead of fair_value directly. AS_GAMMA=0.025 → ±2 tick shift at max position ±80.
    _take_mispriced and _flatten_at_fair still use true fair value (10000).
    Rationale: inventory-adjusted price center clears position faster via price shift,
    complementing the existing size skew. Conservative gamma to test direction.
    """
    ...
    AS_GAMMA = {"ASH_COATED_OSMIUM": 0.025}
    ...
    def _trade_osmium(self, state: TradingState, fair_value, current_mid, last_mid) -> List[Order]:
        ...
        # Avellaneda-Stoikov reservation price: shift quote center based on inventory.
        # Long position → center shifts down (want to sell). Short → center shifts up (want to buy).
        projected = position + pending_buys - pending_sells
        reservation_price = fair - projected * self.AS_GAMMA[product]
