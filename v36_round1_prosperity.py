...
class Trader:
    """
    Round 2 Strategy — v36
    ============================================
    Assets: ASH_COATED_OSMIUM (Pegged/Stationary), INTARIAN_PEPPER_ROOT (Trend Carry)

    Changes from v35:
      1. PEPPER: use microprice (volume-weighted mid) instead of raw mid for pepper_base
         init and pepper_mid. EDA showed microprice predicts next tick 6.3% better than
         raw mid for PEPPER across all 3 days. Formula:
           wmid = (bid_p1 * ask_v1 + ask_p1 * bid_v1) / (bid_v1 + ask_v1)
      2. ASH: remove osmium_ema, use exact FV=10000 directly. EDA confirmed ASH has no
         drift — median = 10000.5 across all days. EMA just adds lag/noise.
    """
    ...
    @staticmethod
    def _microprice(depth: OrderDepth):
        """Volume-weighted mid (Stoikov microprice).
        wmid = (bid_p1 * ask_v1 + ask_p1 * bid_v1) / (bid_v1 + ask_v1)
        Falls back to raw mid when one side is missing or volumes are zero.
        """
        best_bid = max(depth.buy_orders) if depth.buy_orders else None
        best_ask = min(depth.sell_orders) if depth.sell_orders else None
        ...
        if best_ask is None:
            return float(best_bid)
        bid_v = depth.buy_orders.get(best_bid, 0)
        ask_v = abs(depth.sell_orders.get(best_ask, 0))
        total_v = bid_v + ask_v
        if total_v <= 0:
            return (best_bid + best_ask) / 2
        return (best_bid * ask_v + best_ask * bid_v) / total_v
    ...
    def run(self, state: TradingState):
        ...
        # PEPPER: use microprice instead of raw mid for better FV estimation in thin books
        pepper_depth = state.order_depths.get("INTARIAN_PEPPER_ROOT")
        pepper_mid = self._microprice(pepper_depth) if pepper_depth else None
        ...
        # ASH: exact FV=10000, no EMA. EDA confirms no drift; EMA only adds lag noise.
        osmium_depth = state.order_depths.get("ASH_COATED_OSMIUM")
        osmium_mid = self._mid_price(osmium_depth) if osmium_depth else None
        osmium_fair = float(self.FAIR_VALUE["ASH_COATED_OSMIUM"])
