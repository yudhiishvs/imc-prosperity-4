...
class Trader:
    ...
    OSMIUM_DEV_SCALE = 6.7093          # normalization: deviation of this many ticks = "fully skewed"
    OSMIUM_DEV_POWER = 1.3197          # shape of skew curve: 1=linear, 2=quadratic, 3=cubic, etc.
    OSMIUM_UNWIND_CEILING = 1.5965     # max volume multiplier on the side moving TOWARD 10k
    OSMIUM_ACCUM_FLOOR = 0.8730       # min volume multiplier on the side moving AWAY from 10k

    # Inventory gate (hysteresis — pulls all accumulation-side quotes when deeply offside)
    OSMIUM_KILL_QUOTE_THRESHOLD = 76  # |pos| at which we PULL all accumulation-side quotes
    OSMIUM_RESUME_QUOTE_THRESHOLD = 71 # |pos| at which we RESUME quoting both sides again
