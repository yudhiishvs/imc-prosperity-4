...
class Trader:
    ...
    PEPPER_SLOPE = 0.001
    PEPPER_ADJUSTED_HISTORY_LENGTH = 40
    ...
    ASH_PASSIVE_DISTANCE = 4
    IMB_ALPHA = {"ASH_COATED_OSMIUM": 0.25, "INTARIAN_PEPPER_ROOT": 0.25}
    IMB_MAX_TICKS = 4
    ...
    @staticmethod
    def _spread(best_bid: int, best_ask: int):
        if best_bid and best_ask and best_ask >= best_bid:
            return best_ask - best_bid
        return None

    @staticmethod
    def _l1_imbalance(depth: OrderDepth) -> float:
        if not depth.buy_orders or not depth.sell_orders:
            return 0.0
        best_bid = max(depth.buy_orders.keys())
        best_ask = min(depth.sell_orders.keys())
        bid_vol = float(depth.buy_orders.get(best_bid, 0))
        ask_vol = float(abs(depth.sell_orders.get(best_ask, 0)))
    ...
    def _passive_bid_price(self, best_bid: int, best_ask: int, fallback_price: int, max_price: int) -> int:
        price = min(fallback_price, max_price)
        if best_bid > 0:
            improved_price = best_bid + 1
            if improved_price <= max_price and (best_ask == 0 or improved_price < best_ask):
                price = max(price, improved_price)
        if best_ask > 0:
            price = min(price, best_ask - 1)
        return min(price, max_price)

    def _passive_ask_price(self, best_bid: int, best_ask: int, fallback_price: int, min_price: int) -> int:
        price = max(fallback_price, min_price)
        if best_ask > 0:
            improved_price = best_ask - 1
            if improved_price >= min_price and (best_bid == 0 or improved_price > best_bid):
                price = min(price, improved_price)
        if best_bid > 0:
    ...
    def _penny_jump_quotes(
        ...
        imb = self._l1_imbalance(depth)
        spread = self._spread(best_bid, best_ask)
        skew = 0
        if spread is not None:
            raw_skew = self.IMB_ALPHA[product] * imb * spread
            skew = int(round(max(-self.IMB_MAX_TICKS, min(self.IMB_MAX_TICKS, raw_skew))))

        base_bid = fair - self.ASH_PASSIVE_DISTANCE + skew
        base_ask = fair + self.ASH_PASSIVE_DISTANCE + skew
        bid_price = self._passive_bid_price(best_bid, best_ask, base_bid, fair - 1)
        ask_price = self._passive_ask_price(best_bid, best_ask, base_ask, fair + 1)
        ...
        if bid_qty > 0:
            pending_buys = self._place_buy(orders, product, bid_price, bid_qty, position, pending_buys)
        if ask_qty > 0:
            pending_sells = self._place_sell(orders, product, ask_price, ask_qty, position, pending_sells)
        return pending_buys, pending_sells
    ...
    def _trade_pepper_root(self, state: TradingState, adjusted_history: list, base_estimate) -> List[Order]:
        ...
        spread = self._spread(best_bid, best_ask)
        if spread is not None:
            raw_bias = self.IMB_ALPHA[product] * imb * spread
            fair += max(-self.IMB_MAX_TICKS, min(self.IMB_MAX_TICKS, raw_bias))
        signal = float(current_mid) - fair
        target_position = self._pepper_target_position(timestamp)

        if position > target_position:
            reduce_qty = position - target_position
            pending_sells = self._take_bids(
                ...
                position,
                pending_sells,
                max_total=reduce_qty,
            )
        elif position < 0:
            reduce_qty = abs(position)
        ...
        target_gap = target_position - projected_position
        ...
        if signal <= -buy_threshold:
            ...
            passive_ask_floor = int(fair) + 1
            if target_gap >= 6:
                passive_ask_floor += 1
                ask_size = max(1, ask_size - 1)

            bid_price = self._passive_bid_price(best_bid, best_ask, fair_center - bid_distance, passive_bid_cap)
            ask_price = self._passive_ask_price(best_bid, best_ask, fair_center + ask_distance, passive_ask_floor)
